...
export class PortalHubComponent implements OnInit, OnDestroy {
  ...
  // Core System Administrative Portals & Modules (Loaded 100% Dynamically from Menu API)
  corePortals: CorePortalItem[] = [];
  isLoadingPortals: boolean = true;
  ...
  mapApiMenusToPortals(apiItems: any[]): void {
    ...
    sortedItems.forEach(item => {
      const key = item.menuNameE ? item.menuNameE.toLowerCase().trim() : '';
